...
def get_sidebar_recipe_details() -> []:
    ...
    print(recipeTypes)
    print(recipeTypeQuery)
    cuisineTypeQuery = "SELECT cuisineTypeID, cuisineTypeName FROM cuisinetype;"
    cuisineTypes = execute_query(db_connection, recipeTypeQuery).fetchall()
    recipeTypeQuery = "SELECT restrictionID, restrictionName FROM foodstoavoid;"
    avoidTypes = execute_query(db_connection, recipeTypeQuery).fetchall()
...
def getRecipe():
    ...
    # reqImageJSON = {'\"recipe\": {\"recipe_url\": \"' + newUrl + '\"}'}
    # print(reqImageJSON)
    recipeData = requests.post('https://cs361recipescraper.herokuapp.com/scrape', json=reqRecipeJSON)
    # imageData = requests.post('https://cs360-recipe-image-finder.herokuapp.com', json=reqImageJSON)
    # print(imageData.text)
